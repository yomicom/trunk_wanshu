package com.wxb.wanshu.ui.adapter.base;
...
/**
 * Created by qiming on 2017/11/17.
 */
...
public class EasyRVHolder extends RecyclerView.ViewHolder implements ViewHelper.RecyclerView<EasyRVHolder> {
    ...
    public EasyRVHolder setImageUrl(int viewId, String imgUrl, int placeHolderRes) {//placeHolderRes 默认放置图片
        ImageView view = getView(viewId);
...
//        Glide.with(mContext).load(imgUrl).apply(placeholder).into(view);
    ...
    public EasyRVHolder setCircleImageUrl(int viewId, String imgUrl, int placeHolderRes) {
        ImageView view = getView(viewId);
        ImageUtils.displayCircleImage(mContext, view, imgUrl);
        return this;
    }

    public EasyRVHolder setRoundImageUrl(int viewId, String imgUrl, int radius, int placeHolderRes) {
        ImageView view = getView(viewId);
        RequestOptions placeholder = new RequestOptions().placeholder(placeHolderRes).transform(new RoundedCorners(radius));
        Glide.with(mContext).load(imgUrl).apply(placeholder).into(view);
        return this;
    }
